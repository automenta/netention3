...
public class ImportOWL {
    ...
    public void printHierarchy(OWLOntology ontology, OWLClass clazz) throws OWLException {
        ...
        for (OWLDataPropertyExpression dpe : reasoner.getSubDataProperties(reasoner.getTopDataPropertyNode().getRepresentativeElement(), true).getFlattened()) {
            ...
            if (p == null) {
                ...
                p.setDomains(getTypeSet(reasoner.getDataPropertyDomains(dp, true).getFlattened()));
        ...
        for (OWLObjectPropertyExpression ope : reasoner.getSubObjectProperties(reasoner.getTopObjectPropertyNode().getRepresentativeElement(), true).getFlattened()) {
            ...
            Property p = comm.getPropertyByID(iri);
            if (p == null) {
                p = new NodeProp(iri, iri, getTypeSet(domains), getTypeSet(ranges));
    ...
    public void printHierarchy(OWLReasoner reasoner, OWLClass clazz, int level)
        ...
        if (reasoner.isSatisfiable(clazz)) {
            ...
            if (t == null) {
                try {
                    t = comm.save(new NType(clazz.getIRI().toString(), clazz.getIRI().getFragment()));
